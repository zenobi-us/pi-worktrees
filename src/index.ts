...
interface WorktreeSettings extends Omit<WorktreeSettingsConfig, 'onCreate'> {
...
async function handleInit(_args: string, ctx: ExtensionCommandContext): Promise<void> {
  ...
  const onCreate = await ctx.ui.input(
    ...
    currentSettings.onCreate && typeof currentSettings.onCreate === 'string'
      ? currentSettings.onCreate
      : 'mise setup'
  );
